---
title: LangChain
description: Official LangChain integration of Tavily, including both TavilySearch and TavilyExtract tools for web research and content extraction.
---
...
<Callout kind="alert">
The [`langchain_community.tools.tavily_search.tool`](https://python.langchain.com/docs/integrations/tools/tavily_search/) is deprecated. While it remains functional for now, we strongly recommend migrating to the new `langchain-tavily` Python package which supports both Search and Extract functionality and receives continuous updates with the latest features.
</Callout>

The [langchain-tavily](https://pypi.org/project/langchain-tavily/) Python package is the official LangChain integration of Tavily, including both [TavilySearch](#tavily-search) and [TavilyExtract](#tavily-extract).
...
Set your Tavily API key. You can get an API key by visiting [this site](https://app.tavily.com/sign-in) and creating an account.
...
For a comprehensive overview of the available parameters, refer to the [Tavily Search API documentation](/tavily/api-reference/search).
...
- The following arguments can also be set during invocation: `include_images`, `search_depth`, `time_range`, `include_domains`, `exclude_domains`, `include_images`, `start_date`, `end_date`
...
For a comprehensive overview of the available parameters, refer to the [Tavily Extract API documentation](/tavily/api-reference/extract).
