...
description: >-
  Learn how to write effective search queries, tune request parameters, and apply filtering techniques for more relevant results.
canonical: paperguide.ai
---
## Optimize your query

Use short, specific queries that match the information you want the search request to return.
...
- Queries longer than 400 characters return an error.
...
- Split broad research tasks into multiple focused search requests.
...
- Controls the number of retrieved results (default is 5).

- Set this intentionally; large values reduce relevance.
...
### content and search\_depth

- `content`: NLP-generated snippet for quick inspection.

- `search_depth=advanced`: Returns more closely aligned snippets.
...
Use time filters when recency affects relevance.
...
- Defaults may select `advanced` search, which uses 2 credits.
...
Use a clear concurrency pattern to avoid blocking and stay within rate limits.

- Reuse a single `AsyncTavilyClient`.

- Use `asyncio.gather` for parallel search requests.
...
- Prioritize content with strong relevance signals.
...
Sort by score, set thresholds, and combine metadata when you rank results.
...
<Callout kind="info" collapsed="false">
  Refine results by combining metadata analysis, regex extraction, and LLM reasoning. This produces more reliable, task-relevant output.
</Callout>

## Recommended integrations

Use these integrations to apply these practices in production:

- **OpenAI**: Use Tavily as a tool or function in chat completions.
