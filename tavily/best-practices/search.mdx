...
description: >-
  Learn how to optimize your queries, refine search filters, and leverage
...
For efficient processing, keep your query concise—under **400 characters**. Think of it as a query for an agent performing a web search, not long-form prompts. If your query exceeds the limit, you'll see this error:
...
For more information on this two-step process, please refer to the [Best Practices for the Extract API](/tavily/best-practices/extract#2-two-step-process-search-then-extract).
...
- **`raw_content`**: Provides the extracted content from the web page, allowing deeper analysis. If the `content` does not provide enough information, raw content can be useful for further filtering and ranking. You can also use the Extract API with a two-step extraction process. For more information, see [Best Practices for Extract API](/tavily/best-practices/extract#2-two-step-process-search-then-extract).
...
## Recommended integrations for search

Several integrations make it easier to apply these best practices in real applications. Three popular options are:

1. **OpenAI** – Great when you already use OpenAI models and want a simple way to call Tavily as a tool/function from chat completions.

2. **LangChain** – Helpful for building agents, tools, and RAG pipelines where Tavily is one of several tools in a chain.

3. **LlamaIndex** – Well-suited for index-based RAG setups where Tavily fetches fresh web data that you then index or combine with existing documents.

For concrete examples and code, see:
